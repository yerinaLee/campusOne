...
export default function ProfessorDetail() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () => professorsApi.get(Number(id)),
    enabled: !!id,
  });

  const statusMutation = useMutation({
    mutationFn: () => professorsApi.changeStatus(Number(id), { status: newStatus, reason }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['professors'] }); setShowStatusModal(false); setReason(''); },
  });
  ...
  if (isError || !data) {
    return <div className="p-6 max-w-3xl mx-auto"><p className="text-red-500 text-sm">교수 정보를 불러오는 데 실패했습니다.</p></div>;
  }
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="flex items-start justify-between px-6 py-5 border-b border-gray-100">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">{data.professorNumber}</p>
          </div>
        ...
        <dl className="grid grid-cols-2 gap-x-8 gap-y-5 px-6 py-5">
          ...
          <Field label="직위"           value={data.position} />
          <Field label="이메일"         value={data.email} />
          <Field label="전화번호"       value={data.phone} />
          <Field label="연구분야"       value={data.researchField} />
          <Field label="임용일"         value={data.hireDate} />
          <Field label="연구실 위치"    value={data.officeLocation} />
          <Field label="연구실 전화"    value={data.officePhone} />
        </dl>
      </div>
      ...
      {showStatusModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6">
            <h2 className="text-base font-bold text-gray-900 mb-4">교수 상태 변경</h2>
            ...
            {statusMutation.isError && <p className="mt-2 text-xs text-red-500">변경에 실패했습니다.</p>}
